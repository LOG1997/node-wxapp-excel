...
exports.xlsx = async (req, res) => {
  ...
  workbook.eachSheet((worksheet, sheetId) => {
    ...
    for (let i = 0; i < row2.values.length + 1; i++) {
      ...
      if (cell_data) {
        let replaceStr = cell_data.match(regStr);
        let newStr = cell_data.replace(replaceStr, today.toString());
        worksheet.getRow(2).getCell(i + 1).value = newStr;
      }
      // if (cell_data) {
